	...
	public class ExportDataViewModel : BaseViewModel
	{
		...
		public void LoadData()
		{
			Task.Run(() =>
			{
				...
				{
					foreach (var item in environments.Environments.OrderBy(x => x.Workspace).ThenBy(x => x.Name))
					{
						envItems.Add(new ExportItemViewModel { Name = item.Workspace + "-" + item.Name, ShouldExport = true });
					}
				}
				if (envItems.Count > 0)
				{
				...
				if (settings.Certificates != null && settings.Certificates.Count > 0)
				{
					List<ExportItemViewModel> certs = settings.Certificates.Select(x => new ExportItemViewModel() { Name = x.Name, ShouldExport = true }).ToList();
					exportData.Add(new ExportCategoryViewModel() { Name = "Certificates", ShouldExport = true, Items = certs });
				}
		...
		private void ExecuteExportData()
		{
			...
			if (path != null)
			{
				...
				if (envs != null)
				{
					...
					var environmentsData = environments.Environments?.Where(x => envs.Any(y => y.Name.Equals(x.Workspace + "-" + x.Name)))?.ToList();
				...
				if (certs != null)
				{
					...
					if (certData != null)
					{
						exportData.Certificates = certData;
					}
				}
